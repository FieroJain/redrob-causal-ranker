...
Files expected in the same folder:
    ...
    candidates.jsonl         (NOT committed to the public repo — see README)
...
if page == "🏠 Overview":
    ...
    if data_loaded and submission_df is not None:
        ...
        st.warning("Point the sidebar paths to your data files to load the demo. "
                    "candidates.jsonl is required for most pages.")
...
# ═════════════════════════════════════════════════════════════
    ...
    if not data_loaded:
        st.warning("Load candidates.jsonl first (sidebar).")
    elif not RANKER_AVAILABLE:
        st.error("Could not import redrob_ranker_v12.py. Make sure it's in the same folder as app.py.")
    else:
        cid_input = st.text_input("Enter a candidate_id (e.g. CAND_0018499)", "CAND_0018499")

        if st.button("▶  Run Scoring Pipeline", type="primary"):
            cand = lookup.get(cid_input)
            if not cand:
                st.error(f"Candidate {cid_input} not found.")
            else:
                profile = cand.get("profile", {})
                career = cand.get("career_history", []) or []
                skills = cand.get("skills", []) or []

                st.markdown(f"## {profile.get('current_title','')}")
                st.caption(
                    f"{cid_input} · {profile.get('location','')} · "
                ...
                time.sleep(0.3)
                result = ranker.structural_score_v12(cand) if hasattr(ranker, "structural_score_v12") \
                    else ranker.structural_score_optimised(cand)
...
# ═════════════════════════════════════════════════════════════
    ...
    if data_loaded and RANKER_AVAILABLE:
